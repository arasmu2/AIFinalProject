        ...
        for n in range(0, num):
            ...
            for i in range(0, 5):
                if (results[i] == 2):
                    ...
                    l_value =  w.lprob.loc[self.unguessed_words[n][i]]  # find probabability set to zero for h1
                    h = h - l_value
...
if __name__ == '__main__':
    ...
    score = np.zeros((N,6), dtype=int)  #keep track of how many guesses it took
    ...
    while (trial < N):
        ...
        w.start_game()
        guesses = ['aahed', 'thing', 'dulce', 'frost', 'queen', 'trunk', 'clamp']
        ...
        for g in guesses:
            ...
            guesses[6 - guesses_left] = high_pct   #stores the guess in the guesses array for next guess
            print('heuristic guess', high_pct)
            if (high_pct == w.active_word):
                print('You are correct!! word = ', high_pct, 'guess num=', 6-guesses_left)
                score[trial][5 - guesses_left] = 1
                break
        ...
        w.reveal_word()
    np.savetxt('wordle_result.txt', score, delimiter=",")
